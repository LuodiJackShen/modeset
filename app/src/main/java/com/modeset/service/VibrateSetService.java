 ...
 */
public class VibrateSetService extends Service {
    @Nullable
    @Override
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        new Thread(new Runnable() {
            ...
            public void run() {//to set model
                ...
                String log = "vibrate is run at " + runTime;
                FileUtil.addToFile(log, "mslog.txt");//写入日志

                stopSelf();
            }
        }).start();
        Toast.makeText(getApplicationContext(), "振动模式已启动", Toast.LENGTH_SHORT)
                .show();
